...
from tm1_git_py.progress_reporting import NoopProgressSink, ProgressEvent, ProgressSink
...
def serialize_model(
    ...
    *,
    ...
    logger.debug(
        "Serializer worker counts resolved cpu_workers=%d io_workers=%d",
        cpu_workers,
        io_workers,
    )

    active_progress_sink = progress_sink if progress_sink is not None else NoopProgressSink()
    ...
    try:
        ...
        serialize_cubes(model.cubes, cubes_dir, max_workers=io_workers)

        processes_dir = dir + '/processes'
        if (model.processes):
            os.makedirs(processes_dir, exist_ok=True)
        serialize_processes(model.processes, processes_dir, max_workers=io_workers)

        chores_dir = dir + '/chores'
        if (model.chores):
            os.makedirs(chores_dir, exist_ok=True)
        serialize_chores(model.chores, chores_dir, max_workers=io_workers)
...
def serialize_dimensions(
    ...
    if process_pool is None or process_pool is _PROCESS_POOL_UNAVAILABLE:
        for dim, job in dimension_jobs:
            hierarchy_results = [
                _serialize_hierarchy_job(hierarchy_job)
    ...
    for dimension_id, (dim, job) in enumerate(dimension_jobs):
        ...
        active_progress_sink.on_event(
            ProgressEvent.worker_line(
                current=0,
                total=len(job["hierarchies"]),
                message=f"Serializing dimension {dim.name}",
            )
        )
        for hierarchy_job in job["hierarchies"]:
            future = process_pool.submit(_serialize_hierarchy_job, hierarchy_job)
...
def _serialize_hierarchy_job(job: dict[str, Any]) -> dict[str, str]:
...
def _serialize_store_backed_hierarchy_with_staged_writer(
    *,
    ...
    return writer.serialize_hierarchy_json()
...
def serialize_cubes(cubes: List[Cube], cubes_dir, max_workers: Optional[int] = None):
    logger.debug("Serializing %d cube(s) into '%s'", len(cubes), cubes_dir)
    _run_io_jobs(((_serialize_cube, (cube, cubes_dir)) for cube in cubes), max_workers=max_workers)


def _serialize_cube(cube: Cube, cubes_dir: str) -> None:
    if cube.rules:
...
def serialize_processes(processes: List[Process], process_dir, max_workers: Optional[int] = None):
    logger.debug("Serializing %d process(es) into '%s'", len(processes), process_dir)
    _run_io_jobs(((_serialize_process, (process, process_dir)) for process in processes), max_workers=max_workers)


def _serialize_process(process: Process, process_dir: str) -> None:
...
def serialize_chores(chores: List[Chore], chores_dir, max_workers: Optional[int] = None):
    logger.debug("Serializing %d chore(s) into '%s'", len(chores), chores_dir)
    _run_io_jobs(((_serialize_chore, (chore, chores_dir)) for chore in chores), max_workers=max_workers)


def _serialize_chore(chore: Chore, chores_dir: str) -> None:
    with open(chores_dir + '/' + chore.name + '.json', 'w', encoding='utf-8') as chore_file:
        chore_file.write(chore.as_json())


def _run_io_jobs(jobs, *, max_workers: Optional[int] = None) -> None:
    job_list = list(jobs)
    if not job_list:
        return
    ...
    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        futures = [executor.submit(fn, *args) for fn, args in job_list]
