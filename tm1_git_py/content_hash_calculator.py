"""Plain-sqlite content hash calculator for the export pipeline.
...
It exists so the export pipeline can compute signatures without going through
the ``WorkerDB`` / ``SqliteWorker`` facade that the rest of the writer path
uses, which makes it safe to fan out across worker threads (or processes) that
all read the same SQLite file.

The hash format is intentionally identical to the existing deserializer-side
``sha256-tree-v1`` calculator so signatures produced here are interchangeable
with those produced by
:func:`tm1_git_py.deserializer.recalculate_group_content_signature_parallel`.

Algorithm
---------
For each chunk of rows ordered by identity (using the same ordering and
projection as :func:`tm1_git_py.model.model_store._identity_chunk_query_for_type`):

* a SHA-256 hasher accumulates ``identity_line\n`` for every row, where the
  identity line uses the formatting of
  :func:`tm1_git_py.model.model_store._identity_line_for_type`.

The chunk results are then combined with the same algorithm as
:func:`tm1_git_py.deserializer._combine_parallel_chunk_hashes`.
"""
...
def _combine_chunk_hashes(
    *,
    ...
    Mirrors :func:`tm1_git_py.deserializer._combine_parallel_chunk_hashes`
    so the resulting digest equals the deserializer-side calculator's output.
    """
