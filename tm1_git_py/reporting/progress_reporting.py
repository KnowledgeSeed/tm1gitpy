...
class TqdmProgressSink:
    ...
    def _release_worker_if_final(self, event: ProgressEvent) -> None:
        ...
        if worker_id is None:
            return
        if event.kind != ProgressKind.COMPLETE and (event.current is None or event.total is None):
            return
    ...
    def reset_bars(self) -> None:
        with self._lock:
            for idx, worker_bar in enumerate(self._worker_bars):
                if worker_bar is not None:
                    worker_bar.n = 0
                    worker_bar.desc = f"Worker {idx}"
                    worker_bar.refresh()
            self._worker_bar_index = {}
            self.worker_bar_dict = {}   
            if self._total_bar is not None:
                self._total_bar.n = 0
                self._total_bar.desc = f"Total"
                self._total_bar.refresh()
